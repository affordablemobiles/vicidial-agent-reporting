...
<table>
    ...
    foreach ($data->byInbound()->queues as $queue){
        echo "<tr>\n";
            ...
            echo "<td>" . ( ( $data->byInbound()->byQueue($queue)->getTotalDirectOffered() / $data->byInbound()->byQueue($queue)->getTotalOffered() ) / 100 ) . "%</td>\n";
            echo "<td>" . $data->byInbound()->byQueue($queue)->getTotalAnswered() . "</td>\n";
            echo "<td>" . ( ( $data->byInbound()->byQueue($queue)->getTotalAnswered() / $data->byInbound()->byQueue($queue)->getTotalOffered() ) / 100 ) . "%</td>\n";
            echo "<td>N/A</td>\n";
            echo "<td>" . $data->byInbound()->byQueue($queue)->getTotalOOH() . "</td>\n";
            echo "<td>" . ( ( $data->byInbound()->byQueue($queue)->getTotalOOH() / $data->byInbound()->byQueue($queue)->getTotal() ) / 100 ) . "%</td>\n";
            echo "<td>" . $data->byInbound()->byQueue($queue)->getTotalDrop() . "</td>\n";
            echo "<td>" . ( ( $data->byInbound()->byQueue($queue)->getTotalDrop() / $data->byInbound()->byQueue($queue)->getTotalOffered() ) / 100 ) . "%</td>\n";
